...
export default function Opportunities() {
  ...
  const deleteMutation = trpc.opportunities.delete.useMutation({
    onSuccess: () => { toast.success("Opportunity deleted"); utils.opportunities.list.invalidate(); },
    onError: (err: any) => toast.error(err.message),
  });
                      ...
                      <tr key={opp.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                        ...
                        <td className="p-4">
                          <div className="flex gap-2">
                            <Button variant="ghost" size="sm" onClick={() => setEditingOpp(opp)}><Edit className="h-4 w-4" /></Button>
                            <Button variant="ghost" size="sm" onClick={async () => { if (await confirm("Delete?")) deleteMutation.mutate({ id: opp.id }); }}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                          </div>
                        </td>
                      </tr>
                    ))
