...
export default function Accounts() {
  ...
  const deleteMutation = trpc.accounts.delete.useMutation({
    onSuccess: () => { toast.success("Account deleted"); utils.accounts.list.invalidate(); },
    onError: (err: any) => toast.error(err.message),
  });
                      ...
                      <tr key={account.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                        ...
                        <td className="p-4">
                          <div className="flex gap-2">
                            <Button variant="ghost" size="sm" onClick={() => setEditingAccount(account)}><Edit className="h-4 w-4" /></Button>
                            <Button variant="ghost" size="sm" onClick={async () => { if (await confirm("Delete?")) deleteMutation.mutate({ id: account.id }); }}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                          </div>
                        </td>
                      </tr>
                    ))
