...
export default function StockAdjustments() {
  ...
  const approveMutation = trpc.stockAdjustments.approve.useMutation({
    onSuccess: () => { toast.success("Adjustment approved and applied"); setViewingAdjustment(null); utils.stockAdjustments.list.invalidate(); utils.inventory.list.invalidate(); },
    onError: (err: any) => toast.error(err.message),
  });
  const rejectMutation = trpc.stockAdjustments.reject.useMutation({
    onSuccess: () => { toast.success("Adjustment rejected"); setViewingAdjustment(null); utils.stockAdjustments.list.invalidate(); },
    onError: (err: any) => toast.error(err.message),
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <DetailDialog
        ...
        onOpenChange={(open) => !open && setViewingAdjustment(null)}
        title={viewingAdjustment?.itemName || `Item #${viewingAdjustment?.itemId}`}
        subtitle={viewingAdjustment?.itemSku}
        headerRight={viewingAdjustment ? statusBadge(viewingAdjustment.status) : undefined}
        sections={[
          {
            title: "Adjustment",
            fields: [
              { label: "Previous Quantity", value: viewingAdjustment?.previousQuantity },
              { label: "New Quantity", value: viewingAdjustment?.newQuantity },
              {
                label: "Change",
                value: viewingAdjustment ? (
                  <span className={viewingAdjustment.adjustmentQuantity > 0 ? "text-green-400" : viewingAdjustment.adjustmentQuantity < 0 ? "text-red-400" : "text-muted-foreground"}>
                    {viewingAdjustment.adjustmentQuantity > 0 ? "+" : ""}{viewingAdjustment.adjustmentQuantity}
                  </span>
                ) : undefined,
              },
              { label: "Reason", value: viewingAdjustment?.reason },
              { label: "Requested By", value: viewingAdjustment?.requestedByName },
              { label: "Date Requested", value: viewingAdjustment?.createdAt ? new Date(viewingAdjustment.createdAt).toLocaleDateString() : undefined },
            ],
          },
          {
            title: "Approval",
            fields: [
              { label: "Approved By", value: viewingAdjustment?.approvedByName },
              { label: "Approved At", value: viewingAdjustment?.approvedAt ? new Date(viewingAdjustment.approvedAt).toLocaleDateString() : null },
            ],
          },
          {
            title: "Notes",
            fields: [{ label: "Notes", value: viewingAdjustment?.notes, full: true }],
          },
        ]}
        footerLeft={
          viewingAdjustment?.status === "pending" && isAdmin ? (
            <>
              <Button size="sm" variant="outline" className="border-border text-green-400 hover:text-green-300" onClick={() => approveMutation.mutate({ id: viewingAdjustment.id })} disabled={approveMutation.isPending}>
                <Check className="h-4 w-4 mr-2" /> Approve
              </Button>
              <Button size="sm" variant="outline" className="border-border text-red-400 hover:text-red-300" onClick={() => rejectMutation.mutate({ id: viewingAdjustment.id })} disabled={rejectMutation.isPending}>
