...
export default function Activities() {
  ...
  const deleteMutation = trpc.activities.delete.useMutation({
    onSuccess: () => { toast.success("Activity deleted"); utils.activities.list.invalidate(); },
    onError: (err: any) => toast.error(err.message),
  });
                  ...
                  {isLoading ? (
                    ...
                    activities?.map((act: any) => (
                      <tr key={act.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                        ...
                        <td className="p-4">
                          <Button variant="ghost" size="sm" onClick={async () => { if (await confirm("Delete?")) deleteMutation.mutate({ id: act.id }); }}>
